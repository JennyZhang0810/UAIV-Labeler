...
| **在线演示** | 快速查看平台界面和流程 | 访问 `http://8.137.184.86`，只能读取该演示服务器上的数据 | 使用演示服务器已配置的后端 |
...
- **SQLite 侧向索引**：JSON 仍作为可读、可迁移的主存储，`data/index.sqlite3` 作为可重建索引，用于更快筛选、状态统计和后续多人协作升级。
- **多任务统一**：支持目标检测、分割、OCR、事件 QA、图像复原配对、城市结构理解等任务。
- **人工几何标注**：支持水平框、旋转框和逐点点击生成多边形区域，适配无人机遥感目标与区域标注。
- **半自动标注**：支持 YOLO、SegEarth、SAM 类服务、OCR、VLM 或自定义 HTTP 模型后端。
- **QA 与 Benchmark 友好**：支持 TSV QA 题目核查、XLSX 评测结果复盘，以及 JSON、COCO、VOC、YOLO、YOLO-OBB、DOTA、GeoJSON、Mask PNG、QA JSONL 导出。
- **质控与数据卡**：支持生成 Metadata/路径/低置信度质控报告，并导出 Markdown Dataset Card，服务论文、开源 README 和数据发布页。
...
离线 Demo 是一套完整的本机流程：本机启动、导入示例数据、人工标注、保存和导出。它与在线演示、服务器部署分开描述，避免混淆。
...
离线 Demo 中可直接试用水平框、旋转框和逐点多边形区域标注。
...
生成文件位于 `dist/uaiv-labeler-offline-demo-<version>.tar.gz`，会排除运行日志、私有上传数据、生成的导出文件、可重建 SQLite 索引和模型权重。
...
- [文档导航：先看这里](docs/00_先看这里_文档导航.md)
- [最近一周平台改造进度速览](docs/01_最近一周平台改造进度速览.md)
- [正式标注前准备清单](docs/03_正式标注前准备清单.md)
- [低空城市治理标注 5 分钟上手说明](docs/02_标注员5分钟上手说明_城市治理.md)
